 ...
 *
 * remember: 컴포지션에 저장된 객체 (리컴포지션 발생해도 상태 유지시킴)
 * */
@Composable
 ...
 * 단일 소스 저장소: 상태를 복제하는 대신 컴포저블의 호출자로 위치를 끌어올렸기 때문에 소스 저장소가 하나만 있다. (버그 방지에 도움)
...
/**
 * State를 어느 위치로 끌어올려야 하는지
 *
 * 상태는 적어도 그 상태를 사용하는 모든 컴포저블의 가장 낮은 공통 상위 요소로 끌어올려야 합니다. (get)
 * 상태는 최소한 변경될 수 있는 가장 높은 수준으로 끌어올려야 합니다. (set)
